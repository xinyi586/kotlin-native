...
extern "C" RUNTIME_NOTHROW OBJ_GETTER(AllocInstance, const TypeInfo* typeInfo) {
    ...
    auto* object = threadData->allocatorThreadQueue().AllocateObject(typeInfo);
    RETURN_OBJ(object);
}
...
extern "C" OBJ_GETTER(AllocArrayInstance, const TypeInfo* typeInfo, int32_t elements) {
    ...
    auto* threadData = mm::ThreadRegistry::Instance().CurrentThreadData();
    auto* array = threadData->allocatorThreadQueue().AllocateArray(typeInfo, static_cast<uint32_t>(elements));
